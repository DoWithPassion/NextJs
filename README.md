...
### Deployment Options
1. Standard Build
    - This build can be generated by running `next build` command.
    - Produces optimized production bundles and a server-side app which requires nodejs server.
    - Pages are prerendered if possible but the nodejs server is required for API routes, server-side pages and page revalidations.
    - Re-deploy needed in the situation where we require some code changes or we dont use any revalidations to make the page updates.
2. Full Static Build
    - This build can be generated by running `next export` after running `next build`.
    ...
    - Re-deploying is needed for all the code as well as content changes.

### Deployment steps & considerations
1. Add page metadata, optimize code as much as possible, remove unnecessary dependencies.
2. Use environment variable for variable data eg: database credentials, API keys and so on.
